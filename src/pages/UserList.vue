<template>
  ...
  <Panel header="Danh sách tài khoản">
    <div class="p-fluid">
      <DataTable :loading="isLoading" :value="list" :paginator="true" stripedRows :rows="10"
        :rowsPerPageOptions="[10, 25, 50]" :rowHover="true" v-model:filters="filters"
        paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
        :globalFilterFields="['fullname', 'username']" currentPageReportTemplate="Có tổng số {totalRecords} tài khoản"
        columnResizeMode="fit | expand">
        <template #header>
          <div class="p-d-flex p-jc-between p-ai-center">
            <Button type="button" icon="pi pi-filter-slash" label="Xoá" class="p-button-outlined" @click="clearFilter()"
              style="width: 6em" />
        ...
        <Column header="Chỉnh sửa" style="width: 200px">
          <template #body="slotProps">
            <Button type="button" label="Chỉnh sửa" icon="pi pi-pencil"
              @click="loadDataUser(slotProps.data.id)"></Button>
          </template>
        </Column>
      ...
      <div class="p-field p-col p-col-12 p-md-12 p-lg-12">
        <Dialog v-model:visible="updateReg" :style="{ width: '70vw' }"
          :breakpoints="{ '1500px': '50vw', '1150px': '60vw' }">
          <Panel header="Cập nhật tài khoản">
            <div class="p-fluid p-formgrid p-grid">
              ...
              <div class="p-field p-col p-col-12 p-md-4 p-lg-3">
                ...
                <Dropdown id="permission" v-model="userUpdate.permission" :options="userPermission" optionLabel="label"
                  optionValue="value" placeholder="Vui lòng chọn quyền hạn của tài khoản ...">
                </Dropdown>
              </div>
            </div>
            <!--          <div style="width: 20vw">-->
            <Button label="Cập nhật" icon="pi pi-sync" iconPos="left" @click="doUpdate()"
              style="width: 15em; margin-right: 2vw; margin-bottom: 20px" />

            <Button label="Xoá" class="p-button-danger" icon="pi pi-user-minus" iconPos="left"
              style="width: 15em; margin-bottom: 20px" @click="del($event)" />
          </Panel>
        </Dialog>
      </div>
    </div>
